...
from .events import BaseEvent, EventReporter
...
OnResultF = Callable[[IvcapResult | ExecutionError, str, str | None], None]
...
def verify_result(result: Any, job_id: str, logger) -> IvcapResult | ExecutionError:
    if not result:
        return ExecutionError(
            jschema="urn:ivcap:schema.service.error.1",
            error="service did not return a result",
            type="no-result-error",
            traceback=None,
        )
    ...
    if isinstance(result, BinaryIO):
        ...
        return IvcapResult(
            content=result, content_type="application/octet-stream", raw=result
        )
...
def push_result(
    result: IvcapResult | ExecutionError, job_id: str, authorization: str | None = None
):
    ...
    if ivcap_url is None:
        ...
        return
    url = f"{ivcap_url}/results/{job_id}"
    ...
    while attempt < MAX_DELIVER_RESULT_ATTEMPTS:
        try:
            response = httpx.post(url=url, headers=headers, content=content)
...
class SidecarReporter(EventReporter):
    ...
    def _send(self, event: BaseEvent):
        ...
        if self.job_authorization not in (None, ""):
            headers["Authorization"] = self.job_authorization
        try:
            m = event.model_dump()
            data = json.dumps(m)
            response = httpx.post(url=url, headers=headers, content=data)
            response.raise_for_status()
        except Exception as e:
            logger.warning(f"could not deliver event - {e}")


def get_ivcap_url() -> str | None:
    """
