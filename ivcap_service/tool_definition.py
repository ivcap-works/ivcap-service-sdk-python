...
class ToolDefinition(BaseModel):
    ...
    service_id: str = Field(
        validation_alias="service-id",
        serialization_alias="service-id",
    )
    description: str
    fn_signature: str
    fn_schema: dict
...
def create_tool_definition(
    ...
    *,
    ...
    signature, description = _generate_function_description(
        fn, name, exclude_types=[ExecutionContext]
    )
    ...
    if input_type is None:
        raise ValueError(
            f"Cannot create tool definition for '{fn.__name__}': it has no Pydantic input model"
        )
...
def _generate_function_description(
    func: Callable, name: str | None = None, exclude_types: list | None = None
) -> tuple[str, str]:
    ...
    def is_excluded_type(param_type):
        ...
        for exclude_type in exclude_types:
            ...
            if (
                ...
                and isinstance(exclude_type, type)
                and issubclass(param_type, exclude_type)
            ):
                return True
    ...
    for param_name, param in signature.parameters.items():
        if param_name != pydantic_param_name:
            ...
            if is_excluded_type(param_type) or param_name in exclude_types:
                continue
            param_type_name = str(getattr(param_type, "__name__", param_type))
    ...
    for field_name, field_info in model_schema["properties"].items():
        ...
        if field_type is None and hasattr(pydantic_model_class, "__annotations__"):
            ...
            if "default" in field_info or field_name not in model_schema.get(
                "required", []
            ):
                is_optional = True
    ...
    return_type_str = str(getattr(return_type, "__name__", return_type))
