...
def publish_artifact(
    ...
    *,
    ...
    if callable(data_or_lambda):
        ...
        fhdl: IOWritable = get_config().IO_ADAPTER.write_artifact(
            mime_type,
            name=name,
            metadata=metadata,
            seekable=seekable,
...
def publish_metadata(
    entity_id: str, metadata: MetaDict, schema: Optional[str] = None
) -> str:
...
def fetch_data(
    url: Url, binary_content=True, no_caching=False, seekable=False
) -> IOReadable:
    ...
    """
    return get_config().IO_ADAPTER.read_artifact(
        url, binary_content, no_caching, seekable
    )
