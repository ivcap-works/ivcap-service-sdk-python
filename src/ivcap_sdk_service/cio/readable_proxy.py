    ...
    def as_local_file(self) -> str:
        ...
        if self._will_delete_on_close:
            ...
            fp2 = tempfile.NamedTemporaryFile(
                fp.mode, encoding=self._encoding, delete=False
            )
            fname = fp2.name
    ...
    def _open_file_obj(self, delete_on_close=True):
        ...
        if self._download_url:
            ...
            self._file_obj = tempfile.NamedTemporaryFile(
                mode, encoding=self._encoding, delete=delete_on_close
            )
            self._closed = False
            self._path = self._file_obj.name
            self._will_delete_on_close = delete_on_close
            try:
                (self._content_type, cache_id) = download(
                    self._download_url, self._file_obj, close_fhdl=False
                )
                if cache_id:
            ...
            logger.debug(
                "ReadableProxy#_open_file_obj: Successfully read external content '%s' into '%s'",
                self._download_url,
                self._path,
            )
