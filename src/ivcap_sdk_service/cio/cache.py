...
class Cache:
    ...
    def __init__(self, cache_dir: str) -> None:
        ...
        self.cacheIO = LocalIOAdapter(in_dir=cache_dir, out_dir=cache_dir)
    ...
    def get_and_cache_file(self, url: Url) -> IOReadable:
        ...
        if self.cacheIO.readable_local(cname):
            logger.debug(
                "Cache#get_and_cache_file: Hit! '%s' already cached as '%s'", url, cname
            )
            return self.cacheIO.read_local(cname)
        else:
            logger.debug(
                "Cache#get_and_cache_file: Cache '%s' locally as '%s'", url, cname
            )
